...
def time_limit(seconds):
    ...
    try:
        ...
        signal.alarm(0)
   '''     		
class JavaCompiler160(Compiler):
	...
	#	print "\nStarting the Ecodena Judge Now...\n\n"
		...
		compile_command = "javac -d " +path + " " + newfilepath + " 2> "+compileerrorfile
		...
		#	print "\nThe code compiled successfully"
			run_command = "java -cp " + path + " Main <"+ inputdata  +" > "+useroutput+ " 2> "+runerrorfile
		#	print "\nNow we will run the code: " + run_command
			memCommand = "ulimit -t " + str(int(timelimit)) + " ; /usr/bin/time -f '%M' java -cp " + path + " Main <"+ inputdata  +" > "+useroutput+ " 2> "+runerrorfile
		#	print memCommand
		...
		#		print "3"
				...
				timeEnd = time.time()
				if (timeEnd - timeStart) > 10 * timelimit:
					tleerror = 1
...
#			print runMem
			runMem = (int)(runMem)
			if runMem > 30000:
				message = "Your code encountered an error while running. The returned signal was " + `errorcode` + ". Please search the internet for a list of all signals. java.lang.StackOverflowError : Memory Limit Exceeded\n"
				errorReport.errorType_f = 2
				errorReport.errorMessage_f = message
				return errorReport
			temp.close()
#			print runMem
			timeStart = time.time()
