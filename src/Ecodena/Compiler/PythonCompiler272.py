...
def time_limit(seconds):
    ...
    try:
        ...
        signal.alarm(0)
   '''     		
class PythonCompiler272(Compiler):
	...
	#	print "\nStarting the Ecodena Judge Now...\n\n"
		...
		compile_command = "python -m py_compile  " + usercode + " 2> "+compileerrorfile
		...
		if(compile_return == 0):
			...
			run_command = "python " +usercode + " < "+ inputdata  +" > "+useroutput+ " 2> "+runerrorfile
		#	print "\nNow we will run the code: " + run_command
			memCommand = "ulimit -t " + str(int(timelimit)) + " ; /usr/bin/time -f '%M' python " +usercode + " < "+ inputdata  +" > "+useroutput+ " 2> "+runerrorfile
			print memCommand
		...
		#	print "8"
			...
			if not return_value == 0 and errorcode == 0:
		#		print errorcode
				if tleerror == 1:
...
#			print runMem
			...
			if not return_value == 0:
				if runMem > 30000:
					errorReport.errorType_f = 2
					message =   "Your code encountered an error while running. The returned signal was " + `errorcode` + ". Please search the internet for a list of all signals. \nError - Your code consumed too much memory on running"
